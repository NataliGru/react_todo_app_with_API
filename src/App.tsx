...
import { Header, TodoErrors, TodoFooter, TodoList } from './components';
...
export const App: React.FC = () => {
  ...
  const handleAddTodo = (newTodo: Todo) => {
    ...
    setIsLoadingTodoIds((prev) => [...prev, newTodo.id]);
    setTempTodo(newTodo);
  ...
  const handleDeleteCompletedTodo = async () => {
    ...
    await Promise.allSettled(completedIds.map((id) => handleDeleteTodo(id)));
  };
  ...
  const handleAllToggle = async () => {
    ...
    await Promise.allSettled(todosToToggle.map((todo) => handleToggle(todo)));
  };
